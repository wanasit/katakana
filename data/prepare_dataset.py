...
def join_title_datasets(ja_titles_filename, en_titles_filename, output_filename):
    ...
    with open(ja_titles_filename) as input_file:
        ...
        for row in reader:
            url = row[0]
            ja_word = row[1]
            if KATAKANA_TITLES_PATTERN.match(ja_word.decode('utf8')):
                mapping[url] = ja_word
        ...
        with open(output_filename, 'w') as output_file:
            ...
            for row in reader:
                ...
                if url in mapping and ENGLISH_TITLES_PATTERN.match(en_word) and en_word not in NONE_ENGLISH_NAMES:
                    ja_word = mapping[url]
                    writer.writerow([en_word, ja_word])
                    count += 1
                    if count % 5000 == 0:
...
join_title_datasets('japanese_titles.csv', 'english_titles.csv', 'joined_titles.csv')
print('Done!!')
